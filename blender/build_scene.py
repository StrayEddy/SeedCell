...
def bread_mat():
	...
	ramp.color_ramp.elements[0].color = (0.30, 0.15, 0.06, 1)   # toasted
	ramp.color_ramp.elements[1].position = 0.75
	ramp.color_ramp.elements[1].color = (0.56, 0.34, 0.14, 1)   # golden
...
# ---- imported machine parts (real dims) -----------------------------------
for pname, mat in [("MouthDie", mat_steel), ("CookBarrel", mat_wall), ("Piston", mat_steel)]:
	o = imp(pname)
	if o:
		set_mat(o, mat); smooth(o)

# ---- the flatbread, recessed IN the mouth (the delivery moment) ------------
...
bread.location = (bt * 0.6, 0, 0)          # nested just inside the mouth plane
bpy.ops.object.transform_apply(rotation=True)
bev = bread.modifiers.new("edge", "BEVEL"); bev.width = 0.004; bev.segments = 3
...
add_area("rim", (0.2, 1.1, 1.0), 70.0, (0.45, 0.6, 1.0), 0.9)         # cool rim on the steel die
gp = bpy.data.lights.new("mouthglow", "POINT")
gp.energy = 10.0; gp.color = (1.0, 0.52, 0.2); gp.shadow_soft_size = 0.06
gpo = bpy.data.objects.new("mouthglow", gp)
bpy.context.collection.objects.link(gpo)
gpo.location = (0.05, 0, 0)
...
cd.dof.use_dof = True
cd.dof.focus_distance = (Vector((bt * 0.6, 0, 0)) - cam.location).length
